import * as React from "react";
import './ECommerceHomePage.css'
import Header from "../Header/Header.jsx";
import Footer from "../Footer/Footer.jsx";
import TopHeader from "../Top Header/TopHeader.jsx"

const ECommerceHomePage=(props) =>{
  return (
    <>
      <div className="div">
       <TopHeader/>
        <Header value={0}/>
        <div className="div-22" />
        <div className="div-23">
          <div className="div-24">
            <div className="div-25">
              <div className="div-26">
                <div className="div-27">Woman’s Fashion</div>
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/419e3d65c94f39310352312d26156e850a5fd9835e5ec39a019f5290ed34bf71?"
                  className="img-4"
                />
              </div>
              <div className="div-28">
                <div className="div-29">Men’s Fashion</div>
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/419e3d65c94f39310352312d26156e850a5fd9835e5ec39a019f5290ed34bf71?"
                  className="img-5"
                />
              </div>
              <div className="div-30">Electronics</div>
              <div className="div-31">Home & Lifestyle</div>
              <div className="div-32">Medicine</div>
              <div className="div-33">Sports & Outdoor</div>
              <div className="div-34">Baby’s & Toys</div>
              <div className="div-35">Groceries & Pets</div>
              <div className="div-36">Health & Beauty</div>
            </div>
            <div className="div-37" />
            <div className="div-38">
              <div className="div-39">
                <div className="column">
                  <div className="div-40">
                    <div className="div-41">
                      <img
                        loading="lazy"
                        srcSet="..."
                        className="img-6"
                      />
                      <div className="div-42">iPhone 14 Series</div>
                    </div>
                    <div className="div-43">Up to 10% off Voucher</div>
                    <div className="div-44">
                      <div className="div-45">
                        <div className="div-46">Shop Now</div>
                        <div className="div-47" />
                      </div>
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/7a9f9190215df855e46de68e25db2cebf222e4b26191868239034c2f0ce1ceb8?"
                        className="img-7"
                      />
                    </div>
                  </div>
                </div>
                <div className="column-2">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-8"
                  />
                </div>
              </div>
            </div>
          </div>
          <div className="div-48">
            <div className="div-49">
              <div className="div-50">
                <div className="column-3">
                  <div className="div-51">
                    <div className="div-52">
                      <div className="div-53" />
                      <div className="div-54">Today’s</div>
                    </div>
                    <div className="div-55">Flash Sales</div>
                  </div>
                </div>
                <div className="column-4">
                  <div className="div-56">
                    <div className="div-57">
                      <div className="div-58">Days</div>
                      <div className="div-59">03</div>
                    </div>
                    <div className="div-60" />
                    <div className="div-61">
                      <div className="div-62">Hours</div>
                      <div className="div-63">23</div>
                    </div>
                    <div className="div-64" />
                    <div className="div-65">
                      <div className="div-66">Minutes</div>
                      <div className="div-67">19</div>
                    </div>
                    <div className="div-68" />
                    <div className="div-69">
                      <div className="div-70">Seconds</div>
                      <div className="div-71">56</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="div-72">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/6d46c131187bfff9eb633481579a064341b51d7196040ee40dd3f9577e445a5e?"
                className="img-9"
              />
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/e88e31fcac886e936832d43b7fb2b7a3e219274da66d8e9d07a08a6cc7094c1b?"
                className="img-10"
              />
            </div>
          </div>
          <div className="div-73">
            <div className="div-74">
              <div className="div-75">
                <div className="div-76">
                  <div className="div-77">-40%</div>
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-11"
                  />
                </div>
                <div className="div-78">
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/00d93adab53c5214ab1a164999c542db25c2e68622e0085e7c9140fbeae9a9e5?"
                    className="img-12"
                  />
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/92f95af307a693492ef165c4482565d88008392560df9e3855fc15ed6903d028?"
                    className="img-13"
                  />
                </div>
              </div>
              <div className="div-79">HAVIT HV-G92 Gamepad</div>
              <div className="div-80">
                <div className="div-81">$120</div>
                <div className="div-82">$160</div>
              </div>
              <div className="div-83">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/f79bd71a6471f38d5d1fc5e45c151fa99346fc4a5342fd2b25d87f1e68ade395?"
                  className="img-14"
                />
                <div className="div-84">(88)</div>
              </div>
            </div>
            <div className="div-85">
              <div className="div-86">
                <img
                  loading="lazy"
                  srcSet="..."
                  className="img-15"
                />
                <div className="div-87">Add To Cart</div>
              </div>
              <div className="div-88">AK-900 Wired Keyboard</div>
              <div className="div-89">
                <div className="div-90">$960</div>
                <div className="div-91">$1160</div>
              </div>
              <div className="div-92">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/c35d903ef32712f74f1d691c7776b06b738b9c9c861a4482d47d9f9e4b9b6d4a?"
                  className="img-16"
                />
                <div className="div-93">(75)</div>
              </div>
            </div>
            <div className="div-94">
              <div className="div-95">
                <div className="div-96">
                  <div className="div-97">-30%</div>
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-17"
                  />
                </div>
                <div className="div-98">
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/00d93adab53c5214ab1a164999c542db25c2e68622e0085e7c9140fbeae9a9e5?"
                    className="img-18"
                  />
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/92f95af307a693492ef165c4482565d88008392560df9e3855fc15ed6903d028?"
                    className="img-19"
                  />
                </div>
              </div>
              <div className="div-99">IPS LCD Gaming Monitor</div>
              <div className="div-100">
                <div className="div-101">$370</div>
                <div className="div-102">$400</div>
              </div>
              <div className="div-103">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/f79bd71a6471f38d5d1fc5e45c151fa99346fc4a5342fd2b25d87f1e68ade395?"
                  className="img-20"
                />
                <div className="div-104">(99)</div>
              </div>
            </div>
            <div className="div-105">
              <div className="div-106">
                <div className="div-107">-25%</div>
                <img
                  loading="lazy"
                  srcSet="..."
                  className="img-21"
                />
                <div className="div-108">
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/00d93adab53c5214ab1a164999c542db25c2e68622e0085e7c9140fbeae9a9e5?"
                    className="img-22"
                  />
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/92f95af307a693492ef165c4482565d88008392560df9e3855fc15ed6903d028?"
                    className="img-23"
                  />
                </div>
              </div>
              <div className="div-109">S-Series Comfort Chair </div>
              <div className="div-110">
                <div className="div-111">$375</div>
                <div className="div-112">$400</div>
              </div>
              <div className="div-113">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/93b4153214978277fdac264b185811e3ce17a721c815215f2abc8898e8930357?"
                  className="img-24"
                />
                <div className="div-114">(99)</div>
              </div>
            </div>
            <div className="div-115">
              <div className="div-116">
                <div className="div-117">-25%</div>
                <img
                  loading="lazy"
                  srcSet="..."
                  className="img-25"
                />
                <div className="div-118">
                  <div className="div-119">
                    <img
                      loading="lazy"
                      src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                      className="img-26"
                    />
                  </div>
                  <div className="div-120" />
                </div>
              </div>
              <div className="div-121">S-Series Comfort Chair </div>
              <div className="div-122">
                <div className="div-123">$375</div>
                <div className="div-124">$400</div>
              </div>
              <div className="div-125">
                <img
                  loading="lazy"
                  src="https://cdn.builder.io/api/v1/image/assets/TEMP/93b4153214978277fdac264b185811e3ce17a721c815215f2abc8898e8930357?"
                  className="img-27"
                />
                <div className="div-126">(99)</div>
              </div>
            </div>
            <div className="div-127">
              <div className="div-128">
                <div className="div-129">-25%</div>
                <img
                  loading="lazy"
                  srcSet="..."
                  className="img-28"
                />
                <div className="div-130">
                  <div className="div-131">
                    <img
                      loading="lazy"
                      src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                      className="img-29"
                    />
                  </div>
                  <div className="div-132" />
                </div>
              </div>
              <div className="div-133">S-Series Comfort Chair </div>
              <div className="div-134">
                <div className="div-135">$375</div>
                <div className="div-136">$400</div>
              </div>
              <div className="div-137">
                <div className="div-138">
                  <img
                    loading="lazy"
                    src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                    className="img-30"
                  />
                  <img
                    loading="lazy"
                    src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                    className="img-31"
                  />
                  <img
                    loading="lazy"
                    src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                    className="img-32"
                  />
                  <img
                    loading="lazy"
                    src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                    className="img-33"
                  />
                  <div className="div-139">
                    <img
                      loading="lazy"
                      src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                      className="img-34"
                    />
                    <img
                      loading="lazy"
                      src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs="
                      className="img-35"
                    />
                  </div>
                </div>
                <div className="div-140">(99)</div>
              </div>
            </div>
          </div>
          <div className="div-141">View All Products</div>
          <div className="div-142" />
          <div className="div-143">
            <div className="div-144">
              <div className="div-145">
                <div className="div-146" />
                <div className="div-147">Categories</div>
              </div>
              <div className="div-148">Browse By Category</div>
            </div>
            <div className="div-149">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/6d46c131187bfff9eb633481579a064341b51d7196040ee40dd3f9577e445a5e?"
                className="img-36"
              />
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/e88e31fcac886e936832d43b7fb2b7a3e219274da66d8e9d07a08a6cc7094c1b?"
                className="img-37"
              />
            </div>
          </div>
          <div className="div-150">
            <div className="div-151">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/80d1b0ad05383cbd65d8aaeb7e56c7caf06f0be8f8f96c9e6570d64cdf8497d1?"
                className="img-38"
              />
              <div className="div-152">Phones</div>
            </div>
            <div className="div-153">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/3c312d0ab395adde5d1810a14eabb26460626765c3baf5357b24839afc835a52?"
                className="img-39"
              />
              <div className="div-154">Computers</div>
            </div>
            <div className="div-155">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/3d14f0b401c27442e4f2365aaef7fcfba7f96f2b5be48d43b73da4b72255e225?"
                className="img-40"
              />
              <div className="div-156">SmartWatch</div>
            </div>
            <div className="div-157">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/eeffa3ca53e86dbd3ba2800483ed59a159bd8e3e7f35d3b3906fde09bf011c85?"
                className="img-41"
              />
              <div className="div-158">Camera</div>
            </div>
            <div className="div-159">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/8ae323d3abcfde08579c0affe6f6978864eb913eff25d335fcebf26c49d4af17?"
                className="img-42"
              />
              <div className="div-160">HeadPhones</div>
            </div>
            <div className="div-161">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/01c42c4a5580ac36c1a72bf31dca53928ec09d8e6e9dfd969f808ffbf4f8e9ba?"
                className="img-43"
              />
              <div className="div-162">Gaming</div>
            </div>
          </div>
          <div className="div-163" />
          <div className="div-164">
            <div className="div-165">
              <div className="div-166">
                <div className="div-167" />
                <div className="div-168">This Month</div>
              </div>
              <div className="div-169">Best Selling Products</div>
            </div>
            <div className="div-170">View All</div>
          </div>
          <div className="div-171">
            <div className="div-172">
              <div className="column-5">
                <div className="div-173">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-44"
                  />
                  <div className="div-174">The north coat</div>
                  <div className="div-175">
                    <div className="div-176">$260</div>
                    <div className="div-177">$360</div>
                  </div>
                  <div className="div-178">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/f79bd71a6471f38d5d1fc5e45c151fa99346fc4a5342fd2b25d87f1e68ade395?"
                      className="img-45"
                    />
                    <div className="div-179">(65)</div>
                  </div>
                </div>
              </div>
              <div className="column-6">
                <div className="div-180">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-46"
                  />
                  <div className="div-181">Gucci duffle bag</div>
                  <div className="div-182">
                    <div className="div-183">$960</div>
                    <div className="div-184">$1160</div>
                  </div>
                  <div className="div-185">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/52ab2a53bf4326265b0fdc62f376e877615d08fafe742bbdbdc4e9db43243d58?"
                      className="img-47"
                    />
                    <div className="div-186">(65)</div>
                  </div>
                </div>
              </div>
              <div className="column-7">
                <div className="div-187">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-48"
                  />
                  <div className="div-188">RGB liquid CPU Cooler</div>
                  <div className="div-189">
                    <div className="div-190">$160</div>
                    <div className="div-191">$170</div>
                  </div>
                  <div className="div-192">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/52ab2a53bf4326265b0fdc62f376e877615d08fafe742bbdbdc4e9db43243d58?"
                      className="img-49"
                    />
                    <div className="div-193">(65)</div>
                  </div>
                </div>
              </div>
              <div className="column-8">
                <div className="div-194">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-50"
                  />
                  <div className="div-195">Small BookSelf</div>
                  <div className="div-196">$360</div>
                  <div className="div-197">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/f79bd71a6471f38d5d1fc5e45c151fa99346fc4a5342fd2b25d87f1e68ade395?"
                      className="img-51"
                    />
                    <div className="div-198">(65)</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <img
            loading="lazy"
            srcSet="..."
            className="img-52"
          />
          <div className="div-199">
            <div className="div-200">
              <div className="div-201">
                <div className="div-202" />
                <div className="div-203">Our Products</div>
              </div>
              <div className="div-204">Explore Our Products</div>
            </div>
            <div className="div-205">
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/6d46c131187bfff9eb633481579a064341b51d7196040ee40dd3f9577e445a5e?"
                className="img-53"
              />
              <img
                loading="lazy"
                src="https://cdn.builder.io/api/v1/image/assets/TEMP/e88e31fcac886e936832d43b7fb2b7a3e219274da66d8e9d07a08a6cc7094c1b?"
                className="img-54"
              />
            </div>
          </div>
          <div className="div-206">
            <div className="div-207">
              <div className="column-9">
                <div className="div-208">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-55"
                  />
                  <div className="div-209">Breed Dry Dog Food</div>
                  <div className="div-210">
                    <div className="div-211">$100</div>
                    <div className="div-212">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/5959a135433b538e23679e77422b3d816e56dd82cae6866c502d00ccf1435149?"
                        className="img-56"
                      />
                      <div className="div-213">(35)</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="column-10">
                <div className="div-214">
                  <div className="div-215">
                    <div className="div-216">
                      <img
                        loading="lazy"
                        srcSet="..."
                        className="img-57"
                      />
                      <div className="div-217">
                        <img
                          loading="lazy"
                          src="https://cdn.builder.io/api/v1/image/assets/TEMP/00d93adab53c5214ab1a164999c542db25c2e68622e0085e7c9140fbeae9a9e5?"
                          className="img-58"
                        />
                        <img
                          loading="lazy"
                          src="https://cdn.builder.io/api/v1/image/assets/TEMP/92f95af307a693492ef165c4482565d88008392560df9e3855fc15ed6903d028?"
                          className="img-59"
                        />
                      </div>
                    </div>
                    <div className="div-218">Add To Cart</div>
                  </div>
                  <div className="div-219">CANON EOS DSLR Camera</div>
                  <div className="div-220">
                    <div className="div-221">$360</div>
                    <div className="div-222">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/c35d903ef32712f74f1d691c7776b06b738b9c9c861a4482d47d9f9e4b9b6d4a?"
                        className="img-60"
                      />
                      <div className="div-223">(95)</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="column-11">
                <div className="div-224">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-61"
                  />
                  <div className="div-225">ASUS FHD Gaming Laptop</div>
                  <div className="div-226">
                    <div className="div-227">$700</div>
                    <div className="div-228">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/f79bd71a6471f38d5d1fc5e45c151fa99346fc4a5342fd2b25d87f1e68ade395?"
                        className="img-62"
                      />
                      <div className="div-229">(325)</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="column-12">
                <div className="div-230">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-63"
                  />
                  <div className="div-231">Curology Product Set </div>
                  <div className="div-232">
                    <div className="div-233">$500</div>
                    <div className="div-234">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/c35d903ef32712f74f1d691c7776b06b738b9c9c861a4482d47d9f9e4b9b6d4a?"
                        className="img-64"
                      />
                      <div className="div-235">(145)</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="div-236">
            <div className="div-237">
              <div className="column-13">
                <div className="div-238">
                  <div className="div-239">
                    <div className="div-240">
                      <div className="div-241">NEW</div>
                      <img
                        loading="lazy"
                        srcSet="..."
                        className="img-65"
                      />
                    </div>
                    <div className="div-242">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/00d93adab53c5214ab1a164999c542db25c2e68622e0085e7c9140fbeae9a9e5?"
                        className="img-66"
                      />
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/92f95af307a693492ef165c4482565d88008392560df9e3855fc15ed6903d028?"
                        className="img-67"
                      />
                    </div>
                  </div>
                  <div className="div-243">Kids Electric Car</div>
                  <div className="div-244">
                    <div className="div-245">$960</div>
                    <div className="div-246">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/f79bd71a6471f38d5d1fc5e45c151fa99346fc4a5342fd2b25d87f1e68ade395?"
                        className="img-68"
                      />
                      <div className="div-247">(65)</div>
                    </div>
                  </div>
                  <div className="div-248">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/fb6e8d38bbacc36f17a40ba2334ffa6c131c09ffe33e980bd374a2a9a8a45f64?"
                      className="img-69"
                    />
                    <div className="div-249" />
                  </div>
                </div>
              </div>
              <div className="column-14">
                <div className="div-250">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-70"
                  />
                  <div className="div-251">Jr. Zoom Soccer Cleats</div>
                  <div className="div-252">
                    <div className="div-253">$1160</div>
                    <div className="div-254">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/f79bd71a6471f38d5d1fc5e45c151fa99346fc4a5342fd2b25d87f1e68ade395?"
                        className="img-71"
                      />
                      <div className="div-255">(35)</div>
                    </div>
                  </div>
                  <div className="div-256">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/716530fa63e1187abfb9dd15e4f3009b73573691d04de6bc7db0c344f2ea21db?"
                      className="img-72"
                    />
                    <div className="div-257" />
                  </div>
                </div>
              </div>
              <div className="column-15">
                <div className="div-258">
                  <div className="div-259">
                    <div className="div-260">
                      <div className="div-261">NEW</div>
                      <img
                        loading="lazy"
                        srcSet="..."
                        className="img-73"
                      />
                    </div>
                    <div className="div-262">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/00d93adab53c5214ab1a164999c542db25c2e68622e0085e7c9140fbeae9a9e5?"
                        className="img-74"
                      />
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/92f95af307a693492ef165c4482565d88008392560df9e3855fc15ed6903d028?"
                        className="img-75"
                      />
                    </div>
                  </div>
                  <div className="div-263">GP11 Shooter USB Gamepad</div>
                  <div className="div-264">
                    <div className="div-265">$660</div>
                    <div className="div-266">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/93b4153214978277fdac264b185811e3ce17a721c815215f2abc8898e8930357?"
                        className="img-76"
                      />
                      <div className="div-267">(55)</div>
                    </div>
                  </div>
                  <div className="div-268">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/f4c4c0aa1faafc398bfe441523fb5bab4651e4b252fdf8f77c40c3899a01af12?"
                      className="img-77"
                    />
                    <div className="div-269" />
                  </div>
                </div>
              </div>
              <div className="column-16">
                <div className="div-270">
                  <img
                    loading="lazy"
                    srcSet="..."
                    className="img-78"
                  />
                  <div className="div-271">Quilted Satin Jacket</div>
                  <div className="div-272">
                    <div className="div-273">$660</div>
                    <div className="div-274">
                      <img
                        loading="lazy"
                        src="https://cdn.builder.io/api/v1/image/assets/TEMP/93b4153214978277fdac264b185811e3ce17a721c815215f2abc8898e8930357?"
                        className="img-79"
                      />
                      <div className="div-275">(55)</div>
                    </div>
                  </div>
                  <div className="div-276">
                    <img
                      loading="lazy"
                      src="https://cdn.builder.io/api/v1/image/assets/TEMP/7d207a7087afeea2f4b6356f32d983c4dfe221a0d6cfec694954f86acc092cbf?"
                      className="img-80"
                    />
                    <div className="div-277" />
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="div-278">View All Products</div>
          <div className="div-279">
            <div className="div-280" />
            <div className="div-281">Featured</div>
          </div>
          <div className="div-282">New Arrival</div>
          <div className="div-283">
            <div className="div-284">
              <div className="column-17">
                <div className="div-285">
                  <div className="div-286">
                    <img
                      loading="lazy"
                      srcSet="..."
                      className="img-81"
                    />
                    <div className="div-287">PlayStation 5</div>
                    <div className="div-288">
                      Black and White version of the PS5 coming out on sale.
                    </div>
                    <div className="div-289">Shop Now</div>
                  </div>
                </div>
              </div>
              <div className="column-18">
                <div className="div-290">
                  <div className="div-291">
                    <div className="div-292">
                      <img
                        loading="lazy"
                        srcSet="..."
                        className="img-82"
                      />
                      <div className="div-293">Women’s Collections</div>
                      <div className="div-294">
                        Featured woman collections that give you another vibe.
                      </div>
                      <div className="div-295">Shop Now</div>
                    </div>
                  </div>
                  <div className="div-296">
                    <div className="div-297">
                      <div className="column-19">
                        <img
                          loading="lazy"
                          srcSet="..."
                          className="img-83"
                        />
                      </div>
                      <div className="column-20">
                        <img
                          loading="lazy"
                          srcSet="..."
                          className="img-84"
                        />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="div-298">
            <div className="div-299">
              <div className="column-21">
                <div className="div-300">
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/b577eb776407c6950f72d58b75d2ff3f2ac99d2c591cccf697f852c83610ad7a?"
                    className="img-85"
                  />
                  <div className="div-301">FREE AND FAST DELIVERY</div>
                  <div className="div-302">
                    Free delivery for all orders over $140
                  </div>
                </div>
              </div>
              <div className="column-22">
                <div className="div-303">
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/f5a8bcb67852a6927c4dd9c5d7d1072f6a61aa579650280fe8717b84e528aa8a?"
                    className="img-86"
                  />
                  <div className="div-304">24/7 CUSTOMER SERVICE</div>
                  <div className="div-305">Friendly 24/7 customer support</div>
                </div>
              </div>
              <div className="column-23">
                <div className="div-306">
                  <img
                    loading="lazy"
                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/f24d55efa29ff049d4e712bb199ed88698a64dbf08266e5adc6d6c0a572e53ff?"
                    className="img-87"
                  />
                  <div className="div-307">MONEY BACK GUARANTEE</div>
                  <div className="div-308">We reurn money within 30 days</div>
                </div>
              </div>
            </div>
          </div>
          <img
            loading="lazy"
            src="https://cdn.builder.io/api/v1/image/assets/TEMP/4994e2c1a620d4ea356d28611072582355d46de2085b91428063bb54d28d049e?"
            className="img-88"
          />
        </div>
     <Footer/>
      </div>
    
    </>
  );
}


export default ECommerceHomePage